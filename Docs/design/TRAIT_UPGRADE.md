...
**[설계 제안]** 사장님이 확정한 "건물 5칸"은 그대로 두되(다른세계 강화소·영원함 강화소 포함), 유닛강화소 안에서는 **등급 선택 → 그 등급 유닛 목록** 2단 구조가 필요하다고 본다. 다만 이건 `GameHud.cs`(내가 못 건드리는 파일)가 그리드를 어떻게 그리는지와 맞물려 있어서, **UI 쪽 판단은 PM/GameHud 담당과 조율이 필요하다** — 이 문서에서 구조를 확정하지 않는다.
