...
> ⚠️ **구현 참고 — 이 표의 `AddAbility` 목록은 결과유닛(영원한/불멸의/초월함
> 히어로)을 실제로 만들 때 반드시 참고할 것.** CSV `ORIGINAL_CHAT_UNLOCK_TRIGGERS.csv`의
> "결과유닛" 칸은 이름만 담고 그 유닛이 원작에서 받는 능력 목록은 안
> 담았다(칸 자체가 없다 — 뿌리 ㊹와 같은 자리, "표에 없어서 못 봤다"가
> "원작에 없다"는 아니다). Eternal_Yamato(4개)·Eternal_ryokugyu(3개)·
> IM_Kaido(3개) 등 다수 행이 결과유닛 자신에게 `UnitAddAbilityBJ`를
> 붙인다 — "스톡 능력이겠지"로 넘기면 그만큼 빠진다. 지금은 그 히어로
> 에셋 자체가 없어 문제가 안 되지만, 사장님이 유닛 배정할 때 아래 행별
> 표의 AddAbility 열을 먼저 확인할 것.
