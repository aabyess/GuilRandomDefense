...
> ⚠️ 정정 주석(2026-09-06, PM 지시 — 사장님 03번 확정): 랜덤유닛·다른세계를 "등급별 화력
> 사다리"에서 빼기로 하면서, `Tier()`(위 표가 쓰는 `grade.Tier()`)를 직접 바꾸는 안도
> 검토했지만 **채택하지 않았다** — 조밀한 정수열이라 빼면 이 표가 의존하는 서열(해체
> 상한 비교)이 깨진다(`default: -1`이 되면 모든 분해 스킬의 대상이 되어버린다). 그래서
> `Tier()`는 값·순서 전부 그대로 두고, "사다리 소속 여부"만 별도 메서드
> `UnitGradeExtensions.IsLadderGrade()`로 분리했다 — 이 절의 위험 평가(코드 경로는
> 열려 있지만 데이터가 안 겹쳐서 안 문다)는 `Tier()`가 안 바뀌었으므로 그대로 유효하다.
