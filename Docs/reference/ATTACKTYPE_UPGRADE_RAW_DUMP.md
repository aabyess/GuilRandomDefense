...
## 추가(②) 2026-09-06 — 유닛별 `upgr` 1:1 표 (PM 후속 요청)

`war3map.w3u`에서 이 시스템의 자식ID(`R01K`~`R01W`, `R01V` 제외)를 `upgr`(그 유닛에
실제로 적용되는 연구 목록)에 가진 **원작 유닛 218건**을 전수 뽑아
`Docs/reference/ATTACKTYPE_UPGR_PER_UNIT.csv`에 남겼다(이름·등급·공격타입·버킷값·
원문 upgr 그대로). **대조(우리 로스터 `attackType`과 비교)는 안 했다** — PM이 "대조는
구현 쪽이 한다"고 했고, 원작 유닛 이름→우리 로스터 파일 매핑은 이 프로젝트에서 이미
"불가능으로 닫힌" 방법이라(원작 유닛 이름 매핑 불가, 순위 배정이 우회로) 내가 억지로
갖다 붙이면 오히려 근거 없는 매핑을 만드는 꼴이 된다.

### 🔴 중요한 정정 — "등급이 버킷을 정한다"는 가정 자체가 틀렸다

툴팁 문구("히든/전설/랜덤:1500·제한됨/특수함:2000·초월/불멸/영원:4000")가 잔재라는 건
이미 확인했는데, **`upgr` 실측으로 보니 등급 이름 자체도 버킷을 안정적으로 예측 못
한다.** 같은 "전설적인" 등급 안에서도 버킷이 1500(대다수)·2000(패기 3건)이 섞여
나온다. 즉:

- **버킷(1500/2000/3000, 패기는 2000/3000/4000)을 실제로 결정하는 건 `upgr`의
  두 번째 코드(등급트랙ID, 예: `R000`/`R001`/`R002`/`R003`/`R004`/`R017`/`R019`/`R02J`
  등 최소 30종 이상)다** — 이게 "8~9개 등급 이름"보다 훨씬 세분화되어 있다.
- 교차표(등급트랙ID → 버킷):
  - **버킷1**(1500, 패기 2000): `R000`,`R001`,`R019` — 압도 다수
  - **버킷2**(2000, 패기 3000): `R004`,`R006`,`R022`
  - **버킷3**(3000, 패기 4000): `R002`,`R003`,`R005`,`R007`,`R00C`,`R00D`,`R00X`,
    `R013`,`R014`,`R015`,`R016`,`R017`,`R020`,`R023`,`R02J`,`R02O`,`R02R`,`R02W`,`R032`
    — 여기 몰린 코드 수가 훨씬 많다(캐릭터 전용/희귀 사례가 대부분 버킷3에 몰려있다는
    뜻일 수 있다).
  - 위 세 그룹은 정확히 `Trig_upgrade_______u`가 **`R000`/`R004`/`R002`+`R003`
    마스터버튼 하나로 묶어서 자동부여하던 그 하위ID들과 일치한다**(①의 마스터-자식
    분석 참고) — 즉 "등급"이 아니라 **"어느 마스터버튼 묶음에 속하는가"**가 버킷을
    정한다.
- **결론: "등급 → 버킷" 단순 표는 만들 수 없다(만들면 거짓이다).** 원작은 유닛(또는
  그보다 세분화된 개별 트랙) 단위로 배정했다 — 우리도 등급명이 아니라 **유닛 단위로
  대조**해야 한다. `ATTACKTYPE_UPGR_PER_UNIT.csv`의 원문 행을 그대로 대조 기준으로
  쓰시길 권한다.

### 파일 구조 (`ATTACKTYPE_UPGR_PER_UNIT.csv`)

컬럼: `name`(원작 유닛명, 색상코드 제거), `grade`(unam에서 뽑은 등급명, 못 찾으면 빈칸),
`type`(일반/공성/관통/패기), `tier`(버킷 실값), `childId`(자식ID 원문), `gradeTrackId`
(upgr의 두 번째 코드 원문), `raw`(upgr 필드 원문 그대로). 218행, 자식ID 하나당 1행
(한 유닛이 자식ID 하나만 가지면 1행 — 이번 조사에서 유닛당 자식ID 2개 이상은 없었다).

⚠️ 캐릭터 이름 추출은 `unam` 필드를 앞/뒤 인접 탐색으로 찾은 것이다 — 레코드 경계를
완전히 파싱한 게 아니라 근접 탐색이라(이 세션 내내 쓴 방법과 동일), 아주 드물게 인접한
다른 유닛의 이름이 잘못 붙었을 가능성을 완전히 배제 못 한다. 대조 시 이상하게 안 맞는
행이 있으면 그 행만 원문(`raw` 열의 파일 offset, 이 문서 상단 스크립트로 재확인 가능)을
다시 짚어보시라.
...
## 추가(③) 2026-09-06 — `gba2`(1500~4000)는 실제로 유닛 공격력에 더해진다

PM 질의: 구현담당2가 "9건(스킬 자체 계수)과 이중계상 우려"로 `gba2`를 안 옮겼다는데,
등급트랙에서 이미 `gba2`/`gmo2`="유닛 공격력 절대 가산치"로 검증된 바로 그 필드라
같은 뜻 아니냐는 질문. **답: 같은 뜻이 맞고, 이중계상 우려는 근거가 약하다.**

1. **`gef2`는 자식이든 마스터든 등급트랙 확정본(`R000`)이든 전부 `'ratx'`로 동일**하다
   — 구분 신호가 아니라 `Rugf` 템플릿의 상수 잔재다(무엇에도 안 쓰인다).
2. **`gba2`는 자식ID(`R01K` 등)와 마스터(`R00G` 등), 그리고 이미 등급트랙에서 검증된
   `R000`~`R004`가 전부 구조적으로 동일**하다(같은 필드명, 같은 type=2, `level=0`
   단일값, `gmo2`와 값이 완전히 같음 = "레벨1값=레벨당증분" 공식도 동일). 구조가
   똑같은데 뜻만 다르다고 볼 근거가 없다.
3. **레벨 배열**: `gba1`/`gba2`/`gmo1`/`gmo2` 전부 파일에 `level=0` 엔트리 **딱
   하나뿐**이다(레벨1/2/3 각각 다른 값의 배열이 아니다) — `Aegr`처럼 45단으로 쭉
   이어지는 배열이 아니라 **단일 고정값**이다. `glvl=3`은 이 값과 무관한 별도 필드로
   보인다(레벨을 몇 번 올리든 이 값 자체는 안 변한다 — 다만 마스터버튼 연구 자체가
   1회성이라 "레벨"이 실질적 의미가 없을 수 있다, `[미확인]`).
4. **`gba1`/`gmo1`(공속)도 자식에 그대로 있다** — 마스터가 말하는 +3%(일반/공성/관통)
   /+4%(패기)와 **완전히 같은 값**이 각 자식ID 자신에게도 박혀 있다(예: `R01K`
   `gba1`=`gmo1`=0.03).

**이중계상 우려에 대한 직접 반증**: `h05C`(핸콕)의 `upgr`="`R01T`,`R016`"을 보면,
핸콕은 (a) 자신의 스킬(`Trig_hancock_skill_Mana_Func005Func010A`)에서
`360000.00+I2R(GetPlayerTechCountSimple('R01T',...))×30000.00`처럼 **`R01T`의
연구횟수를 스킬 자체 계수로 직접 읽고 있고**, (b) 동시에 **`upgr`에 `R01T`가 있어
엔진 네이티브 공격력 가산(`gba2`=4000)도 같이 받는다.** 즉 **원작 자체가 같은
연구 하나로 "스킬 자체 계수 상승"과 "평타 공격력 상승"을 동시에 준다** — 이건
실수로 겹친 게 아니라 원작 설계다. **단, 이게 모든 유닛에 보편적이진 않다** — `코알라`
(`koalla_skill_Mana`가 `R01O`의 연구횟수를 스킬 계수로 읽는다)는 **자기 `upgr`엔
`R01O`가 없다**(빈 문자열, 여러 번 확인) — 스킬 계수로 그 값을 "빌려 쓸" 뿐 코알라
본인이 `gba2`의 flat 보너스를 받는 유닛은 아니라는 뜻이다.

**결론**:
- `gba2`는 **등급트랙과 똑같이 "유닛 공격력 +N" 엔진 네이티브 가산치**가 맞다.
- 9건(스킬 자체 계수)과 **수학적으로 이중계상이 아니다** — 스킬 공식은 "연구 횟수"라는
  카운터를 직접 읽어 자기 계수를 만드는 것이지 유닛의 최종 공격력 스탯을 참조하는
  게 아니다. 둘은 독립적인 별개 보너스이고, 원작(핸콕 사례)도 둘 다 준다.
- **다만 `gba2`를 "누구에게" 줄지는 공격타입만으로 정하면 안 된다** — 코알라 사례가
  보여주듯 스킬이 어떤 R0XX를 계수로 참조한다고 해서 그 유닛이 자동으로 그 R0XX의
  flat 보너스 수혜자인 건 아니다. **`ATTACKTYPE_UPGR_PER_UNIT.csv`(218행, 유닛별
  실제 `upgr` 원문)을 그대로 배선 기준으로 쓰는 게 맞다** — 공격타입 카테고리 일괄
  적용이 아니라 그 표에 이름이 실제로 있는 유닛에만.
