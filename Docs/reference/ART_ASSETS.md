...
# 원작은 유닛 개성을 **모델이 아니라 붙는 이펙트**로 냈다 (2026-09-05)

`ORD11.089.w3x`를 뜯다가 나온 것이라 위 조사(무료 에셋 조합)와 별개의 축이다. **234종 문제를 원작이 어떻게 넘겼는지**에 대한 답이다.

## 무엇을 찾았나

원작 능력 템플릿 5위였던 `Asph`(70개)는 워크3 스톡 **「Sphere」** — **유닛에 모델 이펙트를 붙이는 시각 전용 능력**이다. 피해도 범위도 마나도 없고, 스크립트가 한 번도 참조하지 않는다.

`war3map.w3a`의 원본 필드가 그것만 있다:

| 필드 | 뜻 |
|---|---|
| `atac` | 부착 개수 |
| `ata0` `ata1` `ata2` | 부착 위치 (`origin`, `hand`, …) |
| **`atat`** | **부착 모델 파일** |
| `amat` `amsp` | 미사일 아트 / 속도 |

**70개 전부 `atat`가 채워져 있고, 61개가 서로 다른 모델이다**:
`HandsAura2.mdx` · `IceNova.mdx` · `Ora_enel.mdx` · `Ora_reliy.mdx` · `Z_Wing3_FIre.mdx` · `flare35.mdx` · `ThunderClapCaster22.mdx` · `BlightwalkerAura.mdx` · `Wave_squished2.mdx` · `banju7.mdx` …

이름도 유닛마다 붙어 있다 — `스피어 불주먹`, `스피어 흰수염 해적단`, `스피어 폭군 쿠마`, `스피어 나미 블리자드이펙`.

## 이게 왜 우리에게 쓸모 있나

**원작 유닛 대부분은 워크3 기본 모델을 그대로 쓴다.** 개성은 **몸통이 아니라 그 위에 붙은 이펙트**가 낸다 — 유닛마다 다른 오라·불꽃·날개·번개가 붙는다.

우리 유닛은 지금 **전부 같은 프리팹**(`UnitPrefab` 하나)이다. 위 조사에서 추천한 **「모듈러 파츠 조합 + 등급별 색 틴트」**와 이 방식은 **경쟁 관계가 아니라 층이 다르다**:

| 층 | 수단 | 다양성 원천 |
|---|---|---|
| 몸통 | 모듈러 파츠 (Quaternius) | 실루엣 |
| 색 | 등급별 틴트 | 등급 판별 |
| **부착 이펙트** | **파티클/VFX를 부착점에 붙임** | **개체 식별** |

**부착 이펙트 층이 제일 싸다.** 모델을 새로 만들지 않고, 파티클 하나를 부착점에 다는 것뿐이다. 원작이 70개를 그렇게 처리했다.

**추천: 부착 이펙트 층을 먼저 넣어라.** 234종에 각각 다른 모델을 구하는 것보다, 같은 몸통 + 등급 틴트 + **유닛별 부착 VFX** 쪽이 1인 개발에서 훨씬 빨리 「달라 보인다」에 도달한다. 파츠 조합은 그 다음에 얹으면 된다.

## 확인 못한 것

- 원작이 몸통 모델을 실제로 몇 종이나 썼는지 세지 않았다 (`umdl` 필드를 세면 나온다). **「대부분 기본 모델」은 표본 관찰이지 전수 확인이 아니다.**
- `atat` 모델 파일들이 워크3 기본 리소스인지 맵 내장 커스텀인지 안 갈랐다.
