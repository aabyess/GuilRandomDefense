...
# 🔴 재실행 금지(2026-09-23, 구현담당2 — PM 지시로 generate 스크립트 ↔ 로스터 대조 중 발견).
# 이 스크립트는 **정본이 아니다.** 로스터 239종의 hp·attackPower·attackRange·attackSpeed·moveSpeed를
# 아래 등급 곡선으로 통째로 덮는다(흔함 공격력 5.0 등). 그런데 지금 로스터 값은 전부 이 곡선 **뒤에** 손으로 들어갔다:
#   · de63a856 (09-04) 유닛 239종 공격력·공속을 원작값으로 교체
#   · d04e91ac (09-05) 사거리에 등급 안 편차
#   · fa005c34 (09-23) 흔함_강재규 공격력 20 → 22(원작 ua1b 20 + 주사위 평균 2)
# 다시 돌리면 이것들이 전부 등급 곡선으로 돌아간다. 스탯의 정본은 이제 **에셋 자체**다.
# (attackType은 이 스크립트가 안 건드린다 — 위 fa005c34의 공격타입 수정은 재실행해도 남는다.)
# 정말로 곡선으로 초기화하려는 것이면(새 로스터를 처음 만들 때뿐) 아래 플래그를 준다.
if '--overwrite-roster-stats' not in sys.argv:
    sys.exit('❌ 재실행 금지: 로스터 239종의 원작 스탯(de63a856·d04e91ac·fa005c34)을 등급 곡선으로 덮는다. '
             '파일 맨 위 주석을 읽고, 정말 초기화할 때만 --overwrite-roster-stats 를 주세요.')
