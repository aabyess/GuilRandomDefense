...
- 우리 게임의 등급은 **대부분 11.089에 있다** (2026-09-04 정정 — 예전엔 "전부 없다"고 적혀 있었다):
  불멸의 16종 · 변화된 9종 · 랜덤전용(=다른세계) 12종 · 초월함위습(`e01A`, 우리 것과 같은
  "공격 못 하는 토큰"). **없는 건 영원함, 그리고 별도 등급으로서의 히든**뿐이다 —
  히든은 희귀함 색 안에 `[히든]` 접두사로 존재한다
- 공격 타입 × 방어 타입 배율표는 `war3mapMisc.txt`에 있고, **3각 상성이 실제로 작동한다**
  (2026-09-04 정정). 예전에 "배율이 전부 1.0이라 작동하지 않는다"고 적었던 건 방어 타입을
  `udea`(정수)에서 읽은 탓이다 — **실제 필드는 `udty`(문자열: normal/fort/large/hero)**이고,
  119종이 값을 명시한다. 보스는 `large` 11/11, 스토리 섬은 `fort` 13/13,
  라인몹 레벨 1~63은 `normal` 55/55다. 상세는 `Docs/reference/ARMOR_SYSTEM_DESIGN.md`
