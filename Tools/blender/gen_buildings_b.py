...
def make_university():
    ...
    # (skip=("bottom",)만).
    # 🔴 2026-09-12 PM 2차 검수(회귀) — 위 수정을 코니스 띠에 옮기면서 실수로 "네 변 테두리"가
    # 아니라 바닥 전체(x0..x1, y0..y1)를 덮는 상자 하나로 지었다. skip="bottom"만 있으니
    # 윗면(z=f3+0.4=55.4)이 옥상 전체를 뚜껑처럼 덮어, 그 아래 건물_옥상_방수(55.0)와
    # 화난판 그을음(55.05)이 통째로 안 보였다(PM이 렌더로 직접 확인해 찾은 회귀). 파라펫과
    # 같은 네 조각 띠로 다시 짓는다 — 옥상 가운데는 비워서 방수 바닥이 그대로 보이게.
    ct = 0.5
    for bx0, bx1, by0, by1 in ((x0 - 0.2, x1 + 0.2, y0 - 0.2, y0 - 0.2 + ct),
                               (x0 - 0.2, x1 + 0.2, y1 + 0.2 - ct, y1 + 0.2),
                               (x0 - 0.2, x0 - 0.2 + ct, y0 - 0.2 + ct, y1 + 0.2 - ct),
                               (x1 + 0.2 - ct, x1 + 0.2, y0 - 0.2 + ct, y1 + 0.2 - ct)):
        b.box(bx0, bx1, by0, by1, f3 - 0.4, f3 + 0.4, white, skip=("bottom",))
    t = 0.5
    for bx0, bx1, by0, by1 in ((x0, x1, y0, y0 + t), (x0, x1, y1 - t, y1),
                               (x0, x0 + t, y0 + t, y1 - t), (x1 - t, x1, y0 + t, y1 - t)):
...
def _angry_ammo_depot_extra(b):
    ...
    # 🔴 2026-09-12 PM 2차 검수 — 기본 depth=2.4·droop=1.2는 초소(폭 6.5, 벽 tx±3.25=
    # −19.25..−12.75)에 비해 너무 커서 outer u(−19.5·−12.5)가 벽 밖으로 0.25씩 튀어나가
    # 날개처럼 보였다. depth=1.0·droop=0.5로 줄이고 outer를 벽 안(−19.0·−13.0)으로 좁힌다.
    b.brow("-y", -15.25, (-19.0, 41.0), (-16.5, 39.5), depth=1.0, droop=0.5)
    b.brow("-y", -15.25, (-13.0, 41.0), (-15.5, 39.5), depth=1.0, droop=0.5)
    ...
    # ⑥ 그을음 — 옅게 띄운 판 세 장. `_잎카드` 재질이라 카메라각 뒷면검사 예외(한 겹으로 충분).
    # 🔴 2026-09-12 PM 2차 검수 — 게임각에서 "가장 크게 보여야 할 파손"인데 hw 3.0·2.6짜리
    # 두 장은 거의 안 읽혔다. 크기를 5.0·4.5로 키우고 한 장(100°, 중앙)을 더해 면적을 넓힌다.
    for ang, cx, hw in ((100.0, 0.0, 4.0), (112.0, -6.0, 5.0), (144.0, 5.0, 4.5)):
    ...
    # 🔴 2026-09-12 PM 2차 검수 — 안쪽 붉은 불빛을 inner+n*0.03(들린 판 자체의 두께 속,
    # inner~outer=0~0.08 사이)에 두면 판 안에 갇혀 어느 쪽에서도 안 보인다. 들린 판이
    # 비워 놓은 자리(원래 돔 곡면 위치 — lift를 안 더한 lo~hi)에 붙여야 그 틈으로 보인다.
    glow_lo, glow_hi = lo + n * 0.03, hi + n * 0.03
    b.face((glow_lo - right * hw2, glow_lo + right * hw2,
            glow_hi + right * hw2, glow_hi - right * hw2), "건물_창_분노")
...
def _angry_japan_extra(b):
    """日本다운 파손 — ⓪ 눈으로 쓴 정면 쇼지 두 칸을 붉게(PM 2차 검수 — 쇼지 종이 재질
    건물_색_흰은 ANGRY_SWAP 대상이 아니라 눈이 그대로 흰색이었다) ① 기와지붕에 구멍(기와
    몇 장 빠진 자리, 어두운 판) ② 그 둘레 기와 몇 장이 흘러내린 듯 살짝 어긋난 hexa 조각.
    앞쪽(−y) 사면(처마→용마루)에 낸다 — 게임 카메라가 내려다볼 때 가장 크게 보이는 자리
    (PM 지시). 좌표는 make_japan과 같은 기준(YSHIFT=3.8, wall_top=27, roof_rise=13 →
    ridge_z=40, ry0=−9.2·cy=3.8, 정면 쇼지 y0=−6.2·u=±8·z=5.0·cols=2·rows=2·w=2.6·h=8.0)."""
    # ⓪ 쇼지 눈 붉히기 — make_japan의 shoji()와 완전히 같은 격자 계산(cw=w/cols=1.3·
    # ch=h/rows=4.0)으로 종이 칸 위치를 그대로 재현해, 그 자리에 건물_창_분노 판을 종이
    # (offset 0.08)보다 살짝 앞·나무틀(depth 0.12)보다는 약간 앞(0.13)에 얹는다 — 창호지가
    # 안에서 붉게 타는 느낌.
    angry_glass = "건물_창_분노"
    y0, w, h, cols, rows = -6.2, 2.6, 8.0, 2, 2
    cw, ch = w / cols, h / rows
    for u in (-8.0, 8.0):
        for c in range(cols):
            for r in range(rows):
                cu = u - w / 2 + cw * (c + 0.5)
                cz = 5.0 + ch * r
                b.panel("-y", y0, cu, cz, cw - 0.15, ch - 0.15, angry_glass, offset=0.13)
