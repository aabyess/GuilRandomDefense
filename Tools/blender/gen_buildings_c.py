...
def make_guil_high():
    ...
    for z in (G,) + tuple(G + FLOOR_H * k for k in (1, 2, 3)):
        # 🔴 정정(카메라각 뒷면 검사) — 띠 윗면을 빼면 벽과 띠 사이 0.2 틈으로 반대편 띠 안쪽(뒷면)이 비쳤다. 윗면을 둔다.
        b.box(x0 - 0.2, x1 + 0.2, y0 - 0.2, y1 + 0.2, z - 1.0, z + 0.4, navy, skip=("bottom",))
    cols = [x0 + 3.0 + 3.08 * k for k in range(13)]
    for i, cx in enumerate(cols):
        # 🔴 정정(카메라각 뒷면 검사) — 핀 윗면을 빼서 비스듬히 내려다보면 속 빈 핀 안쪽(뒷면)이 보였다. 윗면을 둔다.
        b.box_c(cx, y0 - 0.35, 1.0, 0.5, 0.3, top, concrete, skip=("bottom",))
    for k in range(4):
        z = G + FLOOR_H * k
    ...
    ("Story04_구일초등학교_화남", "04 Guil Elementary Angry",
     lambda: bc.angry_variant(make_guil_elementary, seed=1104, cracks=10, smoke=0,
                              ...
                              tilts=[("-y", 5.0, -12.0, 12.0, 47.3, 53.0, 10.0, 0.45),
                                     # 시계탑 모임지붕 처마가 같이 딸려 가 뒤집혔다 — 시계판·흰 테만
                                     ("-y", 5.2, -4.2, 4.2, 65.0, 73.4, 22.0, 0.01, ("건물_시계판", "건물_색_흰"))],
...
def _angry_elementary(b):
    ...
    # 기울어 매달린 시계판은 한 겹 판이라 시계탑 폭 밖으로 삐져나온 부분이 뒤에서 뒷면으로 보였다(카메라각 검사) —
    # 기울어진 네 점 그대로 벽 쪽으로 두께 0.28짜리 쇠 뒤판을 대 닫힌 상자로 만든다(앞면은 시계판과 0.02 떨어뜨림).
    clock_i = b.names.index("건물_시계판")
    for f in [f for f in b.bm.faces if f.material_index == clock_i]:
        front = [v.co + Vector((0.0, 0.02, 0.0)) for v in f.verts]
        back = [v.co + Vector((0.0, 0.30, 0.0)) for v in f.verts]
        if len(front) == 4:
            b.hexa((back[0], back[1], back[2], back[3], front[0], front[1], front[2], front[3]), "건물_금속_회색")
