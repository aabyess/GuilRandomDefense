...
🔴 **이 자로 못 보는 것**(만들면서 아는 사각지대. 나중에 찾는 것보다 훨씬 싸다)
  ...
  4. 🔴 **쏘는 쪽에서만 옷을 뺐지, 맞는 쪽에는 옷이 들어 있다.** 손·아래팔 정점만 쏘지만
     **몸통 BVH는 척추 가중치 면 전부**라 코트·망토·판초가 거기 들어간다.
     ...
     📌 **코트 입은 적(R11·R12·R15 해군 대장)은 이 값이 부풀려 나온다**(2026-09-25). 코트를 Chest에 못박아
        몸통 껍데기의 69%가 코트다 — 박예원 walk +8.3%는 손이 코트와 몸 사이를 지난 것이었다.
        가르는 법: 같은 프레임에서 shells()의 몸통 면 중 **Mantle 재질 면만 빼고** deepest()를 다시 부른다
        (박예원 0.150m → 0.000m). 적 코트는 재질이 따로라 여기선 재질로 가를 수 있다.
