...
SKINS = {
    ...
    # 🔴 rebuild="금지"(2026-09-24) — **커밋본을 만든 건 gen_scan_rig.py다.**
    #    커밋본 정점 27,274·면 52,868·메시 1 == gen_scan_rig 산출물(한 자리도 안 다름).
    #    이 설정은 정점 98,589·면 160,766·메시 13 — **3.6배 다른 모델**이다.
    "전설적인_진연서": dict(
        rebuild="금지",
    ...
    # 🔴 rebuild="금지"(2026-09-24) — **커밋본을 만든 건 gen_scan_rig.py다.**
    #    커밋본 정점 15,132·면 39,657·메시 1 == gen_scan_rig 산출물(한 자리도 안 다름).
    #    이 설정은 `AssertionError: 가중치 없는 뼈 ['RightHand']`로 죽는다. 관문의 실패는 **이것 하나였는데,
    #    고칠 것이 아니라 죽은 중복 설정이었다.** 「실패=썩음」으로 세면 없는 일감이 생긴다.
    "제한_이충민": dict(
        rebuild="금지",
