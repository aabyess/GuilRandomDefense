...
Story01·02는 화풍 견본(blender 세션) — 03~07이 같은 결로 이어간다.
"""
...
def make_heights():
    ...
    그 앞면에 「하이츠」. 옥상은 초록 방수 + 벽돌 난간벽 + 스테인리스 물탱크."""
    b = Builder()
    brick, concrete, white = "건물_벽돌_붉은", "건물_콘크리트", "건물_외벽_흰"
    glass, metal = "건물_유리창", "건물_금속_회색"
    W, D = 34.0, 26.0
    ...
    # 바닥판(주차장 콘크리트)
    b.box(x0 - 0.5, x1 + 0.5, y0 - 3.0, y1 + 0.5, 0.0, 0.3, concrete, skip=("bottom",))
    ...
    b.windows("+y", y1 - 0.5, (-10.0, 10.0), (9.0,), 4.0, 6.0)
    # 위층을 받치는 슬래브
    ...
    for z in floors:
        ...
        for side_x in (-10.0, 10.0):
            # 베란다 — 앞으로 2.5 나온 상자: 아래 벽돌 난간벽, 위는 확장형 통창 + 강철 난간
        ...
        # 옆·뒤 창
        b.windows("-x", x0, (-6.0, 6.0), (z + 5.0,), 4.0, 5.5, sill_mat=white)
        b.windows("+x", x1, (-6.0, 6.0), (z + 5.0,), 4.0, 5.5, sill_mat=white)
        b.windows("+y", y1, (-11.0, -3.5, 3.5, 11.0), (z + 5.5,), 3.6, 4.5, sill_mat=white)
    ...
    b.windows("-y", y0 - 1.0, (0.0,), [z + 8.0 for z in floors], 2.4, 4.0)
    # 1층 현관 — 계단실 밑 유리문(사람 키보다 큰 DOOR_H) + 차양
    ...
    # 옥상 — 초록 방수 바닥, 벽돌 난간벽(윗면 콘크리트 갓), 물탱크
    ...
    # 실외기 둘(옥상)
    for ux in (-12.0, -7.5):
        b.box_c(ux, 8.0, 3.2, 1.6, top, 2.4, "건물_색_흰", skip=("bottom",))
    return b
...
def make_kindergarten():
    ...
    b = Builder()
    ground, white, glass = "건물_바닥_운동장", "건물_외벽_흰", "건물_유리창"
    ...
    for hx0, hx1, wall, roof, rise in houses:
        ...
        # 정면 창 — 1층은 키 큰 통창 둘(가운데 집은 현관이 대신한다), 2층 둘, 박공 속 작은 창 하나
        us = (cx - 3.4, cx + 3.4)
        if cx != 0.0:
            b.windows("-y", y0, us, (6.0,), 4.6, 11.0, sill_mat=white)
        b.windows("-y", y0, us, (G + 4.0,), 4.6, 6.2, sill_mat=white)
        b.panel("-y", y0, cx, body_top + 2.5, 3.0, 3.0, glass)
        # 뒤 창
        b.windows("+y", y1, us, (6.0, G + 4.0), 4.0, 6.0)
    b.windows("-x", -21.0, (4.0, 14.0), (6.0,), 4.6, 11.0, sill_mat=white)
    b.windows("-x", -21.0, (4.0, 14.0), (G + 4.0,), 4.6, 6.2, sill_mat=white)
    b.windows("+x", 21.0, (4.0, 14.0), (6.0,), 4.6, 11.0, sill_mat=white)
    b.windows("+x", 21.0, (4.0, 14.0), (G + 4.0,), 4.6, 6.2, sill_mat=white)

    # 현관 — 가운데 집 1층 유리문(DOOR_H) + 노란 차양 + 흰 기둥 둘
    b.panel("-y", y0, 0.0, 1.2, 7.0, DOOR_H, glass, offset=0.12)
    b.box(-4.8, 4.8, y0 - 3.2, y0, DOOR_H + 1.3, DOOR_H + 2.0, "건물_색_노랑")
    for x in (-4.3, 4.3):
        b.box_c(x, y0 - 2.8, 0.55, 0.55, 1.2, DOOR_H + 0.1, "건물_색_흰", skip=("bottom", "top"))
    # 간판 — 가운데 집 2층 창 위, 박공 밑
    b.sign("큰소망유치원", "-y", y0, 0.0, G + 10.6, "건물_색_노랑", "건물_색_남색", size=2.6, pad=0.6,
           max_width=12.0)
    ...
    b.gable_roof(px - 1.9, px + 1.9, py - 1.9, py + 1.9, deck + 3.2, 2.0, "건물_색_파랑", overhang=0.4, thickness=0.4)
    for dx in (-1.1, 1.1):                                 # 사다리(뒤쪽)
        b.beam((px + dx, py + 1.9, deck + 0.5), (px + dx, py + 5.5, 0.3), 0.3, 0.3, "건물_금속_회색")
    for k in range(1, 5):
        t = k / 5
        z = deck + 0.5 - (deck + 0.2) * t
        b.beam((px - 1.1, py + 1.9 + 3.6 * t, z), (px + 1.1, py + 1.9 + 3.6 * t, z), 0.25, 0.25, "건물_금속_회색")
    ...
    b.box_c(10.0, -14.0, 0.8, 0.8, 0.3, 1.8, "건물_금속_회색", skip=("bottom",))
...
CATALOG = [
    ("Story01_하이츠", "01 Heights", make_heights, "붉은 벽돌 빌라 · 1층 필로티 · 베란다 둘 · 계단실 · 옥상 물탱크 · 간판 하이츠"),
    ("Story02_큰소망유치원", "02 Kindergarten", make_kindergarten, "박공 집 셋(노랑·분홍·하늘 벽 / 빨강·파랑·초록 지붕) · 놀이터 미끄럼틀·시소 · 간판 큰소망유치원"),
]
