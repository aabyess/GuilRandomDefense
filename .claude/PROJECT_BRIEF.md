...
- 유닛 이름은 **우리 게임 이름**으로 (친구들 이름 등) — 원작 이름을 그대로 쓰지 않는다.
  ⚠️ 2026-09-07 사장님 확정: **IP 제약은 없다** — 상업적 사용이 아니고 친구들과 재미로 하는 것이라
  원작 캐릭터 모델·에셋을 그대로 써도 된다. 이름을 바꾸는 건 IP 때문이 아니라 **이 게임의 정체성**이다.
