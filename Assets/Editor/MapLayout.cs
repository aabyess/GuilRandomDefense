...
public static class MapLayout
{
    ...
    public static readonly Island[] Zones =
    {
        ...
        // 🔴 2026-09-24 사장님 지시로 표가 6열 → **8열**이 되면서 가로를 1150 → 1520으로 넓혔다.
        //    폭은 열마다 「그 등급의 최대 재료칸」이 정하는데(안흔함 2칸 ~ 영원 8칸), 8열의
        //    자연 폭 합이 1512.6이라 1150에 넣으면 **글씨가 76%로 줄어 읽기 어려워진다.**
        //    ⚠️ **오른쪽으로만** 넓혔다(중심 137.99 → 182.38). 왼쪽·양쪽으로 넓히면 뽑기섬과
        //       각각 312·127 겹친다. 넓힌 구간(x 1150~1520)은 씬 렌더러 전수로 **바다임을 확인**했다.
        //    표 안의 것(받침 747개·칸벽·글씨)은 전부 섬 좌표에서 유도되므로 **저절로 따라온다.**
        new Island("CombineTable",     182.38f * Scale, -197.54f * Scale, 364.78f * Scale, 361f * Scale, "combine"),
    ...
    public static readonly UnitGrade[] CombineTableGrades =
    {
        ...
        //
        // 🔴 2026-09-24 사장님 지시로 **불멸을 거두셨다** — 「원래 불멸은 조합식이 안 나와 있었어.
        //    초월·불멸은 그냥 세워두기만 할거임」. 위 09-23 지시의 불멸 몫만 취소된 것이고
        //    **영원은 남는다**(사장님이 초월·불멸만 말씀하셨다).
