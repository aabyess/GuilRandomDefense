...
public static class ArtBinder
{
    ...
    public const float UnitHeight = 20f;
    ...
    static readonly (string model, Vector3 euler, float heightScale)[] ModelAdjustments =
    {
        ...
        // 안흔함_박준희(사이렌헤드, glb→fbx 변환본·원본 없음) — 사람형이지만 **예외로 여기 적는다**.
        // 이 파일은 뼈와 메시가 따로 논다(bakesize 실측: 뼈는 세로, 그려지는 메시는 y −12.2~0으로 거꾸로 매달림).
        // AutoUpright는 뼈로 재서 돌리므로 메시가 엎드려 떴다(09-13 units: 가로 32·키 17·앞뒤 95, 머리 +Z).
        // 뒤집기만 하면 머리가 위로 온다 — 표에 회전이 있으면 AutoUpright를 건너뛴다.
        ("안흔함_박준희", new Vector3(180f, 0f, 0f), 1f),
