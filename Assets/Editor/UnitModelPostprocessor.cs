...
public class UnitModelPostprocessor : AssetPostprocessor
{
    ...
    static readonly string[] GenericRigUnits =
    {
        ...
        //
        //   🔴 2026-09-24 — **이 이름을 여기서 빼려면 먼저 결합 자세를 고쳐야 한다.**
        //      이 모델은 뼈대 전체가 엉뚱한 살을 물고 있다(뼈 선분↔살 무게중심 거리 중앙 31.1%,
        //      정상 유닛은 0.5~0.9%). **원본 glb부터 24.0%**라 우리가 만든 결함이 아니다.
        //      그런데도 **화면은 멀쩡하다** — 자기 클립이 그 어긋난 자세로 만들어져 둘이 서로 맞고,
        //      우리 파이프라인이 클립을 프레임별 세계 뼈 행렬로 옮겨 담기 때문이다.
        //      즉 **결합 자세가 「틀렸다」는 건 남의 동작을 입힐 때만 틀린 것**이다.
        //      여기서 빼는 순간 공용 Idle/Move/Attack을 받게 되고, 그때 **비로소 몸이 뒤틀린다.**
        //      고치려면 사실상 다시 리깅해야 하고, 그러면 **그 좀비 동작을 잃는다**
        //      (이 유닛이 가진 유일한 동작이다). 그래서 지금은 안 고치고 Generic으로 둔다.
        //      자세한 수치와 자(尺) 셋: `Tools/blender/bone_bind.py`.
        "안흔함_이호준",
