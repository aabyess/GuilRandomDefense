...
public enum MovementAbility
{
    ...
    // ⚠️ 비행·수상보행 유닛을 처음 만들 때 볼 것(2026-09-23 navlane 실측, outbox 2030): 바다 NavMesh(Sea 영역) 윗면은 y 0으로
    // 보이는 수면(바다_물결, 바다 상자 윗면)과 **같은 높이**라 바다 위 유닛이 물에 잠기진 않는다. 그런데 섬 윗면은
    // y 8(MapLayout.IslandTop = NavMesh 복셀×4 — 복셀보다 얕으면 섬이 바다 영역으로 구워졌다)이고 에이전트 오르기는 0.75라,
    // **바다와 섬 NavMesh는 서로 이어져 있지 않다.** areaMask에 Sea를 넣어도 섬↔바다를 오가지 못하니 NavMeshLink 같은
    // 연결이 따로 필요하다. 2026-09-23 기준 이 두 값을 쓰는 유닛은 0기(Assets/Data 243건 전부 Ground).
    // 2026-09-07 확인(PM 지시, "필드만·아직 없다" 뼈대 구멍 전수 점검) — 로스터 240기 전수
    // 조회 결과 이 값을 쓰는 유닛 0건(movementAbility에 Teleport 비트가 선 자산이 없음).
    // 로빈의 "손날개"(A0FL, ANbl 블링크) 등 텔레포트형 원작 능력이 있는 건 확인됐지만
    // UnitIdentity.hasRobinWingBlessing으로 마커만 남기고 로직은 아직 없다 — 사용 유닛
    // 자체가 없어(로빈도 이 필드를 안 씀, 별도 마커로만 존재) 의도적 미구현으로 남긴다.
    // 실제로 텔레포트형 유닛이 배정되면 그때 로직을 붙인다.
    Teleport = 4,
}
...
public class UnitData : ScriptableObject
{
    ...
    // →AGI×1% 공속, UnitAttacker.PrimaryStatAttackBonus/HeroAttackSpeedMultiplier 참고).
    // None(기본값)이면 strengthPerLevel 등이 전부 0이라 보너스가 안 걸린다 — 초월·영원
    // 33종 외 213종 무영향(회귀 없음).
    //
    // 초월함 25기의 성장곡선(strengthPerLevel 등)은 `c80c531`(2026-09-06)이 이미 원작
    // 분포(STR7:AGI8:INT10)로 배정해뒀다 — `primaryStat`은 그 곡선에서 그대로 유도한다
    // (`Tools/generate_hero_stats.py`, 2026-09-07). ⚠️ 주스탯 결정 우선순위: **이미
    // 배정된 성장곡선이 있으면 곡선이 이긴다.** 어느 stat 값을 읽는 스킬(basis 9/10/11)이
    // 있는지는 완전히 다른 축이다("모양이 같아도 대응은 아니다") — 예를 들어
    // `초월_김민준_AP`(타시기 대응)는 성장곡선이 AGI 지배(0.42/1.70/0.42, 원작 upra=AGI와
    // 일치)라 `primaryStat=Agility`인데, 그의 스킬은 `CasterStrength`를 읽는다. 원작
    // 타시기 자체가 이 구조였다(upra=AGI, 스킬은 STR 값을 읽음) — 어긋남이 아니라
    // 두 축이 원래 독립이라는 뜻이다. 곡선이 아직 없는 유닛을 새로 배정할 때만
    // "스킬이 읽는 스탯 = 주스탯" 휴리스틱을 써라.
    //
    // 🔴 영원(Eternal) 8종은 의도적으로 비워둔다(PM 확정, 2026-09-07) — 원작 초월함
    // 38+영원 1=39기 중 영원 등급에서 스탯을 가진 건 1기(루피 기어피프스)뿐이고, 그
    // 1기가 우리 8기 중 누구에 대응하는지는 이름 매핑이 이미 불가능으로 닫힌 문제라
    // 정할 수 없다. 8기 전부에 뿌리면 원작에 없는 축을 7기에 만드는 것이라 "전부
    // 원작대로" 원칙에 어긋난다. **다음 사람이 "영원이 비었네"로 채우지 말 것.**
    //
    // ⚠️ 원작엔 타시기(STR10/AGI11/INT1 시작, 성장 0.42/1.70/0.42)·야마토(성장 0.21/1.06/
    // 0.21) 2건의 예외가 있고 우리 로스터엔 이름 매핑이 없다(사장님 확정 원칙) —
    // 그래도 `초월_김민준_AP`=타시기, `초월_구주호_AD`=야마토 대응으로 이미 값이
