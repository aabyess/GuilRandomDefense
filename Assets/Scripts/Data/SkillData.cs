...
public class SkillEffect
{
    ...
    //
    // ⚠️ 2026-09-06 리서치담당 발견 + 결론(구현담당3): 실제 "realD=버프개수"로 확인된
    // 6~7건도 **이 필드로 안 담긴다.** 원작 식이 `(1,000,000+realD)×버프개수` 꼴이라
    // "버프개수가 다른 항(1,000,000+realD)에 곱해지는 유일한 인자"인데, 이 필드는
    // `최종피해×(1+factor×버프개수)`(baseline 1 있음, 원작은 baseline이 없다 — 버프
    // 0개면 원작은 결과가 0인데 이 필드는 그대로 최종피해가 나간다) 꼴이라 형태 자체가
    // 다르다. factor 값을 아무리 조정해도 이 곱셈 구조로는 원작 식을 못 만든다.
    // → **결론: 이 필드는 실질적으로 "쓸 데가 없다"** — 6~7건이라는 적은 수 때문에
    // 축을 다시 설계하기보다, 실제 배정될 때 그 유닛 전용으로 개별 처리(basis를 새로
    // 만들거나 코드에 직접 케이스를 추가)하는 편이 싸다. 필드는 지우지 않는다(직렬화
    // 순서, PM 지시) — 나중에 "최종피해×(1+factor×count)" 꼴에 실제로 맞는 사례가
    // 나오면 그때 쓰고, 지금 확인된 6~7건은 이 필드를 거치지 않는다.
