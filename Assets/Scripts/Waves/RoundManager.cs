...
public class RoundManager : MonoBehaviour
{
    ...
    // 🔴 2026-09-25 사장님 확정: **「라운드 넘어갈 때 랜덤유닛 2개만 주는 거임, 선택유닛은
    //    주는 게 아니라」.** 여기는 `Wisp_랜덤유닛`이다(MapGenerator.WireRoundRewardWisp가 배선).
    //
    //    ⚠️ 여기 있던 옛 주석은 「이 위습이 **선택**(Wisp_흔함선택)이라 이 게임이 성립한다」였는데
    //    **사실이 아니었다.** 이 필드는 처음부터 랜덤유닛이었고, 선택 위습은 **다른 곳**에서
    //    나오고 있었다 — `WaveData.wispRewards`(Wave_RoundNN.asset)가 매 라운드 흔함선택을
    //    2~3개 더 얹었다. 그래서 판에 「랜덤유닛 2 · 흔함 선택 2」로 같이 쌓였다.
    //    주석이 **옆 경로가 하던 일을 자기 일로 적고 있었다.** 09-25에 그 경로를 뺐다.
    //
    //    📌 그래서 09-24 승산표의 전제(「보상이 선택이라 조합 성립률이 사실상 100%」)가
    //    무너진다. 이제 매 라운드 몫은 **무작위 2기**뿐이고, 조합은 무작위 뽑기에 걸린다.
    //    승산을 다시 재야 한다 — 기존 표를 그대로 인용하지 말 것.
