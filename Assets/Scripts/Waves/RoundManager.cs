...
public class RoundManager : MonoBehaviour
{
    ...
    // 🔴 이 위습이 **선택**(WispData.isPlayerChoice = 1, 지금은 Wisp_흔함선택)인 것이
    //    이 게임을 성립시킨다. 무작위로 바꾸면 안 된다.
    //
    //    2026-09-24 승산 계산에서 나온 사실이다. 이 게임의 성장 엔진은 강화소도 도박소도
    //    아니라 **조합**이다 — 흔함·안흔함 강화 트랙은 잠겨 있고(hasOriginalResearch=0),
    //    도박은 목재를 쓰는데 목재가 라운드 1~10에 한 개도 안 늘어난다(시작 1개가 전부).
    //    남는 성장 수단이 조합 하나뿐인데, 조합식은 **특정 유닛**을 요구한다
    //    (안흔함은 흔함 2기, 특별함은 안흔함 3기 — 등급마다 다르다).
    //
    //    보상이 **선택**이라 필요한 유닛을 집어 올 수 있어서 조합 성립률이 사실상 100%다.
    //    무작위로 바꾸면 성립률이 제약이 되고, 승산표가 통째로 무너진다
    //    (시작 5기만 무작위인데 그것만으로 안흔함 조합식이 걸릴 확률이 94.4%다 —
    //     그 94.4%에 판 전체를 걸게 되는 셈이다).
